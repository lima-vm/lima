...
@test "block-device sudoers checks warn about omitted grants and temporary grants are removed" {
	create_ramdisk
	configure_block_device_path disk
	local disk="${configured_block_device}"
	configured_block_device="${disk},${disk/\/dev\/disk/\/dev\/rdisk}"
	install_block_device_sudoers
	local readable="${BATS_TEST_TMPDIR}/installed.sudoers"
	# Read the root-only fragment into a file owned by the unprivileged checker.
	# shellcheck disable=SC2024
	sudo cat "${sudoers_file}" >"${readable}"
	limactl sudoers --check --block-device="${configured_block_device}" "${readable}"
	limactl sudoers --check --block-device="${disk/\/dev\/disk/\/dev\/rdisk},${disk},${disk}" "${readable}"
