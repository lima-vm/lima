...
# A retry loses the fragment name and can mask a failed revoke, leaving a root grant behind.
...
teardown() {
	...
	if ! remove_block_device_sudoers; then
		echo "# WARNING: ${sudoers_file} still installed; ${host_disk_device} left attached until the grant is removed" >&2
		return 1
	fi
...
create_ramdisk() {
	host_disk_device=$(sudo hdiutil attach -nomount ram://65536 | awk '$1 ~ /^\/dev\/disk[0-9]+$/ {print $1; exit}')
	[[ -n ${host_disk_device} ]]
}
...
install_block_device_sudoers() {
	local generated="${BATS_TEST_TMPDIR}/block-device.sudoers"
	limactl sudoers --block-device="${configured_block_device}" >"${generated}"
	visudo -cf "${generated}" >/dev/null
	...
	sudo install -o root -g wheel -m 0444 "${generated}" "${sudoers_file}"
	[[ $(stat -f '%Su:%Sg %Lp' "${sudoers_file}") == "root:wheel 444" ]]
}

remove_block_device_sudoers() {
	if [[ -n ${sudoers_file} ]]; then
		sudo rm -f "${sudoers_file}" || return
		sudoers_file=""
	fi
}
...
@test "block-device sudoers checks use the installed fragment and temporary grants are removed" {
	create_ramdisk
	configure_block_device_path disk
	local disk="${configured_block_device}"
	configured_block_device="${disk},${disk/\/dev\/disk/\/dev\/rdisk}"
	install_block_device_sudoers
	run -0 limactl sudoers --check --block-device="${configured_block_device}" "${sudoers_file}"
	run -0 limactl sudoers --check --block-device="${disk/\/dev\/disk/\/dev\/rdisk},${disk},${disk}" "${sudoers_file}"
	run limactl sudoers --check --block-device="${disk}" "${sudoers_file}"
	[[ ${status} -ne 0 ]]
	[[ ${output} == *"only the current user's --block-device entries"* ]]
	[[ ${output} == *"preserve other users' entries manually"* ]]
	local installed="${sudoers_file}"
	remove_block_device_sudoers
	[[ ! -e ${installed} ]]
}
