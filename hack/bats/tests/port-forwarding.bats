...
# Source the port forwarding config (YAML rules + get_host_ipv4).
# The test specs are embedded as YAML comments (e.g. "# forward:", "# ignore:")
# which cannot be parsed by yq, so we use bash regex in parse_test_cases() instead.
# shellcheck source=port-forwarding-config.bash
source "$(cd "$(dirname "${BATS_TEST_FILENAME}")" && pwd)/port-forwarding-config.bash"
...
skip_reason() {
	...
	if [[ $guest_ip == *:* ]] && ! ip -6 addr show lo >/dev/null 2>&1; then
		echo "IPv6 not available on host"
		return 0
	fi
	if [[ $mode == "forward" && -z $host_socket && $host_port -lt 1024 && "$(uname -s)" != "Darwin" ]]; then
		echo "Privileged port ($host_port) forwarding requires macOS"
		return 0
	fi
	if [[ $VM_TYPE == "wsl2" ]]; then
		if [[ $mode == "ignore" && ($guest_ip == "0.0.0.0" || $guest_ip == "127.0.0.1") ]]; then
			echo "Ignore rules for 0.0.0.0/127.0.0.1 not testable on $VM_TYPE"
			return 0
		fi
		# 192.168.5.15 is the SLIRP (user-mode networking) guest IP (see pkg/networks/const.go)
		if [[ $guest_ip == "192.168.5.15" ]]; then
			echo "SLIRP IP 192.168.5.15 not available on $VM_TYPE"
			return 0
		fi
	fi
	...
	Cygwin | Msys)
		if [[ -n $host_socket ]]; then
			echo "Unix socket forwarding not supported on Windows"
			return 0
		fi
		;;
	esac
