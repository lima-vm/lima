...
SETTLE_TIMEOUT=2   # seconds to wait before concluding that a message did not arrive
...
local_setup_file() {
    ...
    # The guestagent polls every 3s and only reports the difference between two
    # snapshots, so a port closed here and rebound by the first test within the
    # same tick produces no event at all. Wait out a full tick before starting.
    ...
    # The watcher discards events recorded before its own startup timestamp, so
    # don't let the first test start a listener until the watcher is ready, or
    # the resulting forwarding event could be discarded as historical.
    local deadline=$((SECONDS + 30))
    until grep -q 'Watching for events' "$BATS_FILE_TMPDIR/watch.log"; do
        if ! kill -0 "$(cat "$BATS_FILE_TMPDIR/watch.pid")" 2>/dev/null; then
            echo "'limactl watch' exited prematurely:" >&2
            cat "$BATS_FILE_TMPDIR/watch.log" >&2
            return 1
        fi
        if ((SECONDS >= deadline)); then
            echo "timed out waiting for 'limactl watch' to become ready" >&2
            return 1
        fi
        sleep 0.2
    done
    # The readiness line is printed even when the instance could not be inspected,
    # in which case the watcher runs forever without tailing anything.
    if grep -q 'Failed to inspect instance' "$BATS_FILE_TMPDIR/watch.log"; then
        echo "'limactl watch' did not attach to instance $NAME:" >&2
        cat "$BATS_FILE_TMPDIR/watch.log" >&2
        return 1
    fi
}
...
vm_type() {
    local cache=$BATS_FILE_TMPDIR/vm_type
    if [[ ! -s $cache ]]; then
        limactl list --format '{{.VMType}}' "$NAME" >"$cache"
    fi
    cat "$cache"
}
...
# expected message. The message travels asynchronously, so keep polling while
# the listener output file is still empty.
assert_guest_received() {
    ...
    while [[ -z $received ]] && ((SECONDS < deadline)); do
        sleep 0.5
        received=$(read_guest_outfile "$port")
    done
...
# Assert that nothing arrived at the guest listener on the given port. Unlike
# assert_guest_received there is no state to poll for, so wait out a settle
# window first: a message that was wrongly forwarded may still be in flight.
assert_guest_received_nothing() {
    local port=$1
    sleep "$SETTLE_TIMEOUT"
    assert_equal "$(read_guest_outfile "$port")" ""
}
