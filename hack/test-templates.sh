...
if [[ -n ${CHECKS["port-forwards"]} ]]; then
	...
	# Guard the .yaml requirement above: an extension that looks like a disk image
	# would produce an "images:" stub instead, silently dropping the rule.
	if [ "$(limactl yq '.portForwards[0].guestPort' <"$tmpfile")" != "8888" ]; then
		ERROR "Template embedding dropped the port forwarding rule from \"$tmpfile\""
		exit 1
	fi
	FILE="${tmpfile}"
	FILE_HOST=$FILE
