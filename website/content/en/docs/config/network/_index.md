...
connect_to_vm_via{"Connect to the VM via"} -- "localhost" --> default["Default"]
  ...
  vm -- "qemu" --> shared["shared (socket_vmnet on macOS, lima-privileged-net on Linux)"]
  connect_from -- "Other VMs" --> userV2["user-v2"]
  connect_from -- "Other hosts" --> bridged["bridged (socket_vmnet on macOS, lima-privileged-net on Linux)"]
```
