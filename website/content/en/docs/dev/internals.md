...
External drivers:
- `lima-driver-<driver-name>.pid`: external driver gRPC server process PID. A single server instance runs per Lima instance and is shared by all callers (CLI, hostagent, inspect). The server is started on first use and stopped only when the instance shuts down.
- `lima-driver-<driver-name>.sock`: Unix socket for gRPC communication with the external driver server.
