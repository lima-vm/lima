...
// FileDescriptorForNetworkInterface returns a file for the given vmnet network.
func FileDescriptorForNetworkInterface(ctx context.Context, vmnetNetwork string) (*os.File, error) {
	...
	network, err := vmnet.RequestNetwork(ctx, vmnetNetwork, vmnetConfig)
	if err != nil {
...
func (i *iface) ReadPackets(v *vmPktDescs, packetCount C.int) (C.int, error) {
	...
	if result := vzvmnet.Return(C.VmnetRead(i.ptr, v.self, &count)); result != vzvmnet.ErrSuccess {
...
func (i *iface) WritePackets(v *vmPktDescs, packetCount C.int) (C.int, error) {
	if result := vzvmnet.Return(C.VmnetWrite(i.ptr, v.self, &packetCount)); result != vzvmnet.ErrSuccess {
...
func startWithNetwork(network unsafe.Pointer, interfaceDesc *xpc.Dictionary) (*iface, error) {
	...
	if vzvmnetResult := vzvmnet.Return(result.vmnetReturn); vzvmnetResult != vzvmnet.ErrSuccess {
