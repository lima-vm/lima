...
func RunMachService(ctx context.Context, serviceName string) (err error) {
	...
	go func() {
		for {
			...
			logrus.Debug("Network change detected; clearing cached VmnetNetworks")
			...
			for vzNetwork, entry := range networkEntries {
				if iface := ifaces.LookupInterface(entry.config.Subnet); iface != nil {
					...
					if iface.Type == syscall.IFT_BRIDGE {
						logrus.Debugf("Interface for subnet %v of 'vmnet: %q' exists; keeping cached VmnetNetwork", entry.config.Subnet, vzNetwork)
						entry.existenceObserved = true
					} else {
						logrus.Debugf("Interface for subnet %v of 'vmnet: %q' is found but not a bridge (type=%d); removing cached VmnetNetwork since it cannot be used", entry.config.Subnet, vzNetwork, iface.Type)
						delete(networkEntries, vzNetwork)
					}
				} else if !entry.existenceObserved {
					if time.Since(entry.lastRequestAt) < 1*time.Minute {
						logrus.Debugf("Interface for subnet %v of 'vmnet: %q' is not found yet; keeping cached VmnetNetwork", entry.config.Subnet, vzNetwork)
					} else {
						logrus.Infof("Interface for subnet %v of 'vmnet: %q' is not found for more than 1 minute; removing cached VmnetNetwork", entry.config.Subnet, vzNetwork)
						delete(networkEntries, vzNetwork)
					}
				} else {
