...
type BalloonConfig struct {
	...
	EnableTrendDetection  bool          // Enable pre-emptive grow on rising PSI trend.
}
...
type BalloonController struct {
	...
	// E10-4: Active connections awareness for gentler shrinking.
	activeConnectionsFn func() int64 // Returns active socket proxy connections; nil = not wired.
}
...
func (c *BalloonController) Evaluate(m *api.MemoryMetrics, bootTime time.Time) BalloonAction {
	...
	// E10-9: PSI trend detection — pre-emptive half-step grow when pressure is rising.
	if c.cfg.EnableTrendDetection && m.PsiMemorySome_60 > 0 {
		if m.PsiMemorySome_10 > m.PsiMemorySome_60*1.5 && m.PsiMemorySome_10 > 0.1 {
			halfStep := c.cfg.MaxMemoryBytes * uint64(c.cfg.GrowStepPercent) / 200
			target := min(c.currentBytes+halfStep, c.cfg.MaxMemoryBytes)
			if target > c.currentBytes {
				return BalloonAction{
					Type: BalloonActionGrow, TargetBytes: target,
					Reason: "rising pressure trend",
				}
			}
		}
	}
	...
	if m.PsiMemorySome_10 < c.cfg.LowPressureThreshold {
		...
		step := c.cfg.MaxMemoryBytes * uint64(c.cfg.ShrinkStepPercent) / 100
		// E10-4: Halve shrink step when socket proxy has active connections.
		if c.activeConnectionsFn != nil && c.activeConnectionsFn() > 0 {
			step /= 2
		}
