...
func TestBalloon_GraduatedPollFailure_TransientNoChange(t *testing.T) {
	...
	// 1-2 failures should not change memory.
	ctrl.RecordPollFailure()
...
// --- E10-4: Balloon↔Auto-Pause coordination tests ---

func TestBalloon_ActiveConnections_HalvesShrinkStep(t *testing.T) {
	// When socket proxy has active connections, shrink step is halved.
	// Use ShrinkStepPercent=50 with currentBytes well above min to get clean numbers.
	cfg := newTestConfig()
	cfg.SettleWindow = 0
	cfg.ShrinkStepPercent = 50                  // 50% of 12GiB = 6GiB step.
	cfg.MinBytes = 1 * 1024 * 1024 * 1024       // 1 GiB.
	cfg.IdleTargetBytes = 12*1024*1024*1024 - 1 // Just below max so current > idle.
	ctrl := NewBalloonController(cfg)
	ctrl.TransitionTo(BalloonStateSteady)
	ctrl.currentBytes = cfg.MaxMemoryBytes // 12 GiB.

	m := idleMetrics()

	// Without active connections — full 6 GiB step.
	actionFull := ctrl.Evaluate(m, time.Now().Add(-6*time.Minute))
	assert.Equal(t, actionFull.Type, BalloonActionShrink)
	fullTarget := actionFull.TargetBytes

	// Reset state for second run.
	ctrl.currentBytes = cfg.MaxMemoryBytes
	ctrl.lowPressureSince = time.Time{}
	ctrl.lastActionTime = time.Time{}
	ctrl.TransitionTo(BalloonStateSteady)

	// With active connections — halved to 3 GiB step.
	ctrl.activeConnectionsFn = func() int64 { return 3 }
	actionHalf := ctrl.Evaluate(m, time.Now().Add(-6*time.Minute))
	assert.Equal(t, actionHalf.Type, BalloonActionShrink)
	halfTarget := actionHalf.TargetBytes

	// Full: 12 - 6 = 6 GiB. Half: 12 - 3 = 9 GiB.
	assert.Equal(t, fullTarget, uint64(6*1024*1024*1024))
	assert.Equal(t, halfTarget, uint64(9*1024*1024*1024))
}

func TestBalloon_ActiveConnections_ZeroNoEffect(t *testing.T) {
	cfg := newTestConfig()
	cfg.SettleWindow = 0
	cfg.ShrinkStepPercent = 50
	cfg.MinBytes = 1 * 1024 * 1024 * 1024
	cfg.IdleTargetBytes = 12*1024*1024*1024 - 1
	ctrl := NewBalloonController(cfg)
	ctrl.TransitionTo(BalloonStateSteady)
	ctrl.currentBytes = cfg.MaxMemoryBytes
	ctrl.activeConnectionsFn = func() int64 { return 0 }

	m := idleMetrics()
	action := ctrl.Evaluate(m, time.Now().Add(-6*time.Minute))
	assert.Equal(t, action.Type, BalloonActionShrink)
	// Full step: 12 - 6 = 6 GiB.
	assert.Equal(t, action.TargetBytes, uint64(6*1024*1024*1024))
}

func TestBalloon_ActiveConnections_NilNoEffect(t *testing.T) {
	cfg := newTestConfig()
	cfg.SettleWindow = 0
	cfg.ShrinkStepPercent = 50
	cfg.MinBytes = 1 * 1024 * 1024 * 1024
...
func TestBalloon_TrendDetection_TriggersPreemptiveGrow(t *testing.T) {
	// Rising PSI trend (avg10 > 1.5*avg60) triggers half-step grow.
	cfg := newTestConfig()
	cfg.EnableTrendDetection = true
	cfg.GrowStepPercent = 20
	ctrl := NewBalloonController(cfg)
	ctrl.TransitionTo(BalloonStateSteady)
	ctrl.currentBytes = 6 * 1024 * 1024 * 1024

	m := idleMetrics()
	m.PsiMemorySome_10 = 5.0 // High current pressure.
	m.PsiMemorySome_60 = 2.0 // Lower baseline → ratio 2.5x > 1.5x threshold.
	m.PsiMemoryFull_10 = 0

	action := ctrl.Evaluate(m, time.Now())
	assert.Equal(t, action.Type, BalloonActionGrow)
	assert.Assert(t, strings.Contains(action.Reason, "rising pressure trend"))
	// Half step: 12GiB * 20% / 2 = 1.2 GiB → target ~7.2 GiB.
	expectedTarget := ctrl.currentBytes + cfg.MaxMemoryBytes*uint64(cfg.GrowStepPercent)/200
	assert.Equal(t, action.TargetBytes, expectedTarget)
}

func TestBalloon_TrendDetection_DisabledByDefault(t *testing.T) {
	// Trend detection disabled → same pressure ratio does NOT trigger pre-emptive grow.
	cfg := newTestConfig()
	cfg.EnableTrendDetection = false
	ctrl := NewBalloonController(cfg)
	ctrl.TransitionTo(BalloonStateSteady)
	ctrl.currentBytes = 6 * 1024 * 1024 * 1024

	m := idleMetrics()
	m.PsiMemorySome_10 = 5.0
	m.PsiMemorySome_60 = 2.0
	m.PsiMemoryFull_10 = 0

	action := ctrl.Evaluate(m, time.Now())
	// Should go through normal high-pressure path, not trend detection.
	assert.Assert(t, action.Reason != "rising pressure trend",
		"trend detection should not trigger when disabled")
}

func TestBalloon_TrendDetection_LowAbsoluteNoTrigger(t *testing.T) {
	// Ratio is high but absolute avg10 < 0.1 → no trigger.
	cfg := newTestConfig()
	cfg.EnableTrendDetection = true
	ctrl := NewBalloonController(cfg)
	ctrl.TransitionTo(BalloonStateSteady)
	...
	m := idleMetrics()
	m.PsiMemorySome_10 = 0.05 // Below 0.1 absolute threshold.
	m.PsiMemorySome_60 = 0.01 // Ratio 5x > 1.5x but absolute too low.
	m.PsiMemoryFull_10 = 0

	action := ctrl.Evaluate(m, time.Now())
	assert.Assert(t, action.Reason != "rising pressure trend",
		"trend detection should not trigger with low absolute pressure")
}
