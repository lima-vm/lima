...
func (t *scpTool) Command(ctx context.Context, paths []string, opts *Options) (*exec.Cmd, error) {
	...
	if runtime.GOOS == "windows" {
		// Strip ControlMaster/ControlPath/ControlPersist so scp invokes ssh
		// without SSH connection multiplexing. Native Windows OpenSSH does
		// not support multiplexing (Unix-domain mux socket), and Cygwin-based
		// ssh has known reliability issues with sftp over a mux socket. See
		// also the equivalent handling in hostagent and limactl shell.
