...
func (l *LimaVzDriver) Stop(ctx context.Context) error {
	...
	if l.machine.CanRequestStop() {
		...
		if err := l.requestStopViaSSH(ctx); err != nil {
			// Fall through to the stop poll regardless: the guest may still be shutting down,
			// and if it is not, the poll below reports a timeout error. Returning nil here
			// would exit the host agent successfully while the VM is still running, which
			// launchd would not restart under KeepAlive{SuccessfulExit=false}.
			logrus.WithError(err).Warn("vz: SSH shutdown request failed; waiting to see whether the VM stops anyway")
		}
	}
