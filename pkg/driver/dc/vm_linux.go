...
// registerVM calls DC to register a VM.
func registerVM(ctx context.Context, distroName string, cpus, memory int) error {
	...
	switch initSystem {
	case "systemd":
		entrypoint = "--entrypoint=/lib/systemd/systemd"
	case "openrc":
		entrypoint = "--entrypoint=/sbin/openrc-init"
	default:
...
func provisionVM(ctx context.Context, instanceDir, instanceName, distroName string, errCh chan<- error) error {
	...
	go func() {
		...
		for {
			...
			if status, err := store.GetDcStatus(instanceName); err == nil &&
...
// unregisterVM calls DC to unregister a VM.
func unregisterVM(ctx context.Context, distroName string) error {
	imageName := distroName
	logrus.Info("Unregistering DC VM")
	out, err := exec.CommandContext(ctx,
		"docker",
		"rm",
		"-f",
