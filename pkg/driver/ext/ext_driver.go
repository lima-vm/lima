...
func (l *LimaExtDriver) validateMountType() error {
...
func (l *LimaExtDriver) FillConfig(cfg *limatype.LimaYAML, _ string) error {
	if cfg.VMType == nil {
...
func (l *LimaExtDriver) AcceptConfig(cfg *limatype.LimaYAML, _ string) error {
	if l.Instance == nil {
	...
	if err := l.Validate(context.Background()); err != nil {
		return fmt.Errorf("config not supported by the EXT driver: %w", err)
	}
