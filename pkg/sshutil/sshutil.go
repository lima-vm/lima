...
func CommonOpts(ctx context.Context, sshExe SSHExe, useDotSSH bool) ([]string, error) {
	...
	if !sshInfo.openSSH.Version.LessThan(*semver.New("8.1.0")) {
		...
		// ssh binaries built without OpenSSL (e.g. `--without-openssl`) do not
		// support the GCM and chacha20-poly1305 ciphers, so only prioritize
		// ciphers that `ssh -Q cipher` actually reports as supported. A nil
		// SupportedCiphers means the query could not be run, so fall back to
		// the historical unconditional behavior instead of dropping the option.
		supported := filterSupportedCiphers(preferred, sshInfo.openSSH.SupportedCiphers)
		if len(supported) == 0 {
...
// too old to support "-Q"). Builds compiled without OpenSSL support
// (`--without-openssl`) omit ciphers like the GCM and chacha20-poly1305
// variants from this list.
