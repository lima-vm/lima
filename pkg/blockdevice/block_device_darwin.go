...
// PrivilegedHelperName is the dedicated helper that opens a host block device
// as root and passes the descriptor back to the unprivileged process.
const PrivilegedHelperName = "lima-privileged-block-device"
...
func ServeSudoOpenBlockDevice(allowedDevicePath string, r io.Reader) error {
	...
	conn, err := dialUnixPeer(req.SocketPath, originalUID)
	if err != nil {
...
func dialUnixPeer(socketPath string, ownerUID uint32) (*net.UnixConn, error) {
	conn, err := net.DialUnix("unix", nil, &net.UnixAddr{Name: socketPath, Net: "unix"})
	if err != nil {
		return nil, err
	}
	// Path checks alone race with a caller replacing its socket. Authenticate
	// the listener before sending even the caller-controlled nonce as root.
	if err := validateUnixPeerUID(conn, ownerUID); err != nil {
		conn.Close()
		return nil, err
	}
	return conn, nil
...
// Sudoers returns the sudoers fragment needed to run the block-device helper
// without prompting for the current user. The helper is intentionally
...
func sudoersForUserAndHelpers(userName string, helperArgs ...[]string) string {
	...
	// Stable ordering lets --check compare grant sets regardless of flag order.
...
func sudoOpenBlockDeviceHelperArgs(devicePath string) ([]string, error) {
	...
	helperPath, err := PrivilegedHelperPath(exe)
	if err != nil {
		return nil, err
	}
	return []string{helperPath, devicePath}, nil
}

// PrivilegedHelperPath also validates the installation before VM startup, so
// setup errors reach the CLI rather than being buried in the hostagent log.
func PrivilegedHelperPath(exe string) (string, error) {
	if err := ValidateExecutable(exe); err != nil {
		return "", err
	}
	helperPath := privilegedHelperPathForExecutable(exe)
	if err := validateSecureHelperPath(helperPath); err != nil {
		return "", fmt.Errorf("block-device helper is missing or not securely installed: %w\nInstall a Lima binary release in a root-owned, non-user-writable prefix such as /opt/lima, then use /opt/lima/bin/limactl directly. No source build is required.\nSetup instructions: https://lima-vm.io/docs/config/disk/#sudoers-setup", err)
	}
	return helperPath, nil
}

// ValidateExecutable checks the executable name expected by helper lookup.
// External drivers use a different install location. Build tags only select limactl's imports;
// the separately shipped lima-driver-vz binary is built without external_vz.
...
func privilegedHelperPathForExecutable(exe string) string {
	return filepath.Join(filepath.Dir(filepath.Dir(exe)), "libexec", "lima", "privileged", PrivilegedHelperName)
}
...
func validateDiskDeviceFile(devicePath string, file *os.File) error {
	...
	if err := validateDiskDeviceMode(devicePath, fi.Mode()); err != nil {
		return err
	}
	expected, err := os.Lstat(devicePath)
	if err != nil {
		return err
	}
	return validateDiskDeviceIdentity(devicePath, fi, expected)
}

func validateDiskDeviceIdentity(devicePath string, actual, expected os.FileInfo) error {
	if err := validateDiskDeviceMode(devicePath, expected.Mode()); err != nil {
		return err
	}
	actualStat, actualOK := actual.Sys().(*syscall.Stat_t)
	expectedStat, expectedOK := expected.Sys().(*syscall.Stat_t)
	if !actualOK || !expectedOK || actualStat == nil || expectedStat == nil || actualStat.Rdev != expectedStat.Rdev {
		return fmt.Errorf("received descriptor does not identify requested device %q", devicePath)
	}
	return nil
}
...
func validateSecureHelperPath(path string) error {
	...
	if err := validateSecurePath(path, root.Uid, root.Gid); err != nil {
		return err
	}
	...
	return validateSecureHelperFile(path, fi)
}

func validateSecureHelperFile(path string, fi os.FileInfo) error {
	// Ancestors need directory permissions; these requirements apply only to
	// the helper leaf that sudo executes.
	if !fi.Mode().IsRegular() {
		return fmt.Errorf("block-device helper %q must be a regular file", path)
	}
	if fi.Mode().Perm()&0o111 == 0 {
		return fmt.Errorf("block-device helper %q must be executable", path)
	}
	return nil
}
...
func validateSecurePath(path string, rootUID, rootGID uint32) error {
	...
	if strings.ContainsAny(path, " \t\r\n,:=\\*?[]!#^") {
		return fmt.Errorf("path %q contains whitespace or sudoers metacharacters", path)
	}
	fi, err := os.Lstat(path)
	if err != nil {
		return err
	...
	if err := validateSecurePathACL(path, fi); err != nil {
		return err
	}
	if path != "/" {
...
func validateSecurePathACL(path string, fi os.FileInfo) error {
	hasACL, err := pathHasExtendedSecurityACL(path)
	if err != nil {
		return err
	}
	if !hasACL {
	...
	return fmt.Errorf("%s %q has extended ACL entries", file, path)
}
...
type extendedSecurityAttrBuffer struct {
	length           uint32
	extendedSecurity attrReference
}

func pathHasExtendedSecurityACL(path string) (bool, error) {
	attrList := unix.Attrlist{
		Bitmapcount: unix.ATTR_BIT_MAP_COUNT,
		Commonattr:  unix.ATTR_CMN_EXTENDED_SECURITY,
	}
	var attrBuf extendedSecurityAttrBuffer
	...
	return attrBuf.extendedSecurity.length > 0, nil
}
...
func Open(ctx context.Context, devicePath, socketPath string) (*os.File, error) {
	...
	if err := cmd.Run(); err != nil {
		...
		return nil, fmt.Errorf("failed to run %v: stdout=%q, stderr=%q: %w. Hint: run `%s sudoers --block-device=%s` and install the generated file; %s", cmd.Args, stdout.String(), stderr.String(), err, os.Args[0], devicePath, sudoers.RegenerateGrantWarning)
	}
