...
func TestSudoersForUserAndHelper(t *testing.T) {
	sudoers := sudoersForUserAndHelper("alice", []string{"/usr/local/libexec/lima/privileged/" + PrivilegedHelperName, "/dev/rdisk2"})
	assert.Equal(t, sudoers, "alice ALL=(root:wheel) NOPASSWD:NOSETENV: /usr/local/libexec/lima/privileged/"+PrivilegedHelperName+" /dev/rdisk2\n")
