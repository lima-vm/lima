...
func Sudoers() (string, error) {
	...
	for _, name := range names {
		...
		for _, daemon := range RequiredDaemons() {
			...
			cmds := []string{cfg.StartCmd(name, daemon)}
			if daemon == LimaPrivilegedNet {
				// One tap device per instance is attached to the bridge; the
				// instance name is the only wildcard, matched positionally so
				// no other argument can be smuggled through it.
				cmds = append(cmds, cfg.TapCmdPattern(name))
				// Pin the helper, so that sudo refuses to run a replaced binary.
				digest, err := cfg.DigestSpec(daemon)
				if err != nil {
					return "", err
				}
				for i := range cmds {
					cmds[i] = digest + " " + cmds[i]
				}
			}
			// Not pinned: a system binary, updated independently of Lima.
			cmds = append(cmds, cfg.StopCmd(name, daemon))
			sb.WriteRune('\n')
