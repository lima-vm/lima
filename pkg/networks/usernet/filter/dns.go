...
func (t *Tracker) SeedLimaInternalDomains(subnet, gatewayIP string) error {
	...
	// Add subnet.lima.internal -> subnet IPs (never expires)
	t.addPreSeededRecord("subnet.lima.internal", subnetIPs)

	// Add host.lima.internal -> Lima gateway (never expires)
...
// IsPreSeeded returns true if the domain was pre-seeded (ExpireAt.IsZero()).
func (t *Tracker) IsPreSeeded(domain string) bool {
	t.mu.RLock()
	defer t.mu.RUnlock()

	record, ok := t.records[strings.ToLower(domain)]
