...
func makeVarRun(ctx context.Context, cfg *networks.Config) error {
	if runtime.GOOS != "darwin" {
		// lima-privileged-net runs as root and creates the varRun directory itself.
		return nil
	}
...
func startNetwork(ctx context.Context, cfg *networks.Config, name string, instances []string) error {
	...
	for _, daemon := range daemons {
		...
		if daemon == networks.LimaPrivilegedNet {
...
// for lima-privileged-net. The tap devices are deleted by the helper when it
// tears the network down, after the last instance using it has stopped.
func ensureTaps(ctx context.Context, cfg *networks.Config, name string, instances []string) error {
	user, err := cfg.User(networks.LimaPrivilegedNet)
	if err != nil {
		return err
	}
	bridge := cfg.BridgeName(name)
	for _, instName := range instances {
		tap := networks.TapName(os.Getuid(), instName, name)
		if tapIsAttached(tap, bridge) {
			continue
		}
		if err := sudo(ctx, user.User, user.Group, cfg.TapCmd(instName, name)); err != nil {
...
// redundant sudo call. The helper revalidates everything, so this is only a
