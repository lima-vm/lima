...
const (
	...
	// gptMinEntrySize and gptMaxEntrySize bound SizeOfPartitionEntry before
	// it sizes the entry buffer and the buffer is indexed. This is a sanity
	// range, not the UEFI rule (128*2^n with no maximum): 128 is the only
	// size seen in practice and 4096 keeps the buffer small. gptMaxTableSize
	// bounds NumberOfPartitionEntries, which only limits the scan loop; UEFI
	// sets no maximum on it either (128 is only the default), so it is capped
	// through the total table size instead.
	gptMinEntrySize = 128
