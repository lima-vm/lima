...
func Patch(ctx context.Context, disk string) error {
	...
	if err = patchMacOSDiskUnprivilegedPhase(ctx, dataDevPath, mnt); err != nil {
		return fmt.Errorf("failed to patch macOS disk (phase 1): %w", err)
	}

	// Fix up the file ownership inside the disk.
	// Invokes sudo.
	if err = patchMacOSDiskPrivilegedPhase(ctx, dataDevPath, mnt); err != nil {
		return fmt.Errorf("failed to patch macOS disk (phase 2): %w", err)
	}

	return nil
}

func patchMacOSDiskUnprivilegedPhase(ctx context.Context, dataSliceDevice, mnt string) error {
	// Enable "noowners" to allow non-root users to write to the mounted volume.
	// The ownership is fixed up in patchMacOSDiskPhase2.
...
func patchMacOSDiskPrivilegedPhase(ctx context.Context, dataSliceDevice, mnt string) error {
	...
	logrus.Infof("Executing command (chowning the newly installed files to root:wheel): \n%v", cmd.Args)
