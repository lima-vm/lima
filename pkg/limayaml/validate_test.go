...
func TestValidateBlockDevicesRequiresMacOSDiskPaths(t *testing.T) {
	y, err := Load(t.Context(), []byte(`
images:
  - location: /
blockDevices:
...
`), "lima.yaml")
	...
	err = Validate(y, false)
	assert.ErrorContains(t, err, "field `blockDevices[0]` must be a macOS disk device path")
	assert.ErrorContains(t, err, "field `blockDevices[1]` must be a macOS disk device path")
	assert.ErrorContains(t, err, "field `blockDevices[2]` must be a macOS disk device path")
}
