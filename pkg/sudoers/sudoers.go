...
const RegenerateGrantWarning = "include every --block-device entry you still need, if any; regenerating the file revokes omitted grants"
...
func ContainsActiveFragment(file, fragment string) bool {
	...
	// rule is not a standalone grant, even when its text matches exactly.
	file = strings.ReplaceAll(withoutCommentLines(file), "\\\n", "")
	fragment = strings.TrimSpace(strings.ReplaceAll(withoutCommentLines(fragment), "\\\n", ""))
	return fragment != "" && strings.Contains("\n"+file+"\n", "\n"+fragment+"\n")
}

// withoutCommentLines supports comparison of Lima-generated fragments. It is
// not a sudoers parser: Lima does not generate #include directives or #uid rules.
func withoutCommentLines(s string) string {
	var active []string
	for line := range strings.SplitSeq(s, "\n") {
		if strings.HasPrefix(strings.TrimSpace(line), "#") {
			// Preserve the line boundary: a comment ends a preceding continuation.
