...
func sudoersAction(cmd *cobra.Command, args []string) error {
	...
	if err := nwCfg.Validate(); err != nil {
		...
		if check {
			logrus.WithError(err).Warn("Network entries omitted from sudoers check; --check did not generate or install a replacement file")
		} else {
	...
	if !includeNetwork {
		logrus.Warn("Generated sudoers file has no network entries; replacing an existing file removes its network grants")
	}
