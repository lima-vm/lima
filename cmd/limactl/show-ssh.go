...
`, limaHome),
	...
	shellCmd.Flags().StringP("format", "f", string(sshutil.FormatCmd), "Format: "+strings.Join(sshutil.FormatStrings, ", "))
	_ = shellCmd.RegisterFlagCompletionFunc("format", func(*cobra.Command, []string, string) ([]string, cobra.ShellCompDirective) {
		return sshutil.FormatStrings, cobra.ShellCompDirectiveNoFileComp
	})
	return shellCmd
}
