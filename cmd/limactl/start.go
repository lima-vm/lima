...
func startAction(cmd *cobra.Command, args []string) error {
	...
	if isOrphanedDriverError(inst.Errors) {
		// The VM driver process is running but the host agent is not — this happens when the
		// host agent exits uncleanly, or when its PID file names a PID that an unrelated
		// process has since been given. Force-stop to clean up the orphaned driver so we can
		// start fresh.
...
// process is running but the host agent is not — the recoverable broken state left behind
// when the host agent is killed before it can stop the VM.
...
// occurs when the PID recorded in ha.pid has been reused by another process.
