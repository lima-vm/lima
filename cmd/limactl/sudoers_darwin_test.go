...
func TestSudoersCheckHintHasBalancedParentheses(t *testing.T) {
	...
	assert.Equal(t, hint, "run `/opt/lima/bin/limactl sudoers --block-device=/dev/rdisk2 >etc_sudoers.d_lima && sudo install -o root etc_sudoers.d_lima \"/etc/sudoers.d/lima\"`; include every --block-device entry you still need, if any; regenerating the file revokes omitted grants")
