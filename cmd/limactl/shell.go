...
func shellAction(cmd *cobra.Command, args []string) error {
	...
	if syncHostWorkdir {
		...
		destDir := filepath.Dir(hostCurrentDir)[1:]
		preRsyncScript := fmt.Sprintf("mkdir -p ~/%s", shellescape.Quote(destDir))
		if err := rsyncDirectory(ctx, cmd, sshExecForRsync, hostCurrentDir, fmt.Sprintf("%s:~/%s", *inst.Config.User.Name+"@"+inst.SSHAddress, shellescape.Quote(destDir)), preRsyncScript); err != nil {
	...
	if syncHostWorkdir {
		...
		askUserForRsyncBack(ctx, cmd, inst, sshExecForRsync, hostCurrentDir, tty)
	}
	return nil
}

func askUserForRsyncBack(ctx context.Context, cmd *cobra.Command, inst *limatype.Instance, sshCmd *exec.Cmd, hostCurrentDir string, tty bool) {
...
func rsyncDirectory(ctx context.Context, cmd *cobra.Command, sshCmd *exec.Cmd, source, destination, preRsyncScript string) error {
	...
	if preRsyncScript != "" {
		rsyncArgs = append([]string{"--rsync-path", fmt.Sprintf("%s && rsync", preRsyncScript)}, rsyncArgs...)
	}
