...
func copyAction(cmd *cobra.Command, args []string) error {
	...
	cpTool, err := copytool.New(ctx, backend, args)
	if err != nil {
		return err
	}
	logrus.Debugf("using copy tool %q", cpTool.Name())

	if !cpTool.IsAvailableOnGuest(ctx) {
		return fmt.Errorf("copy tool %q not available on guest", cpTool.Name())
	}

	copyCmd, err := cpTool.Command(ctx, &copytool.Options{
		Recursive: recursive,
		Verbose:   verbose,
	})
	if err != nil {
