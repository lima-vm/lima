...
// createTap creates a persistent tap device owned by uid.
//
// `ip tuntap` is the obvious way to do this, but it is one of the subcommands
// that busybox's `ip` applet does not implement, so fall back to the
// /dev/net/tun ioctls that iproute2 itself uses. The fallback is second because
// `ip` reports its own failures much more legibly than a bare errno does.
func createTap(ctx context.Context, tap string, uid uint64) error {
	err := run(ctx, "ip", "tuntap", "add", "dev", tap, "mode", "tap", "user", strconv.FormatUint(uid, 10))
	if err != nil {
		logrus.WithError(err).Debugf("Falling back to the /dev/net/tun ioctls to create %#q", tap)
		if err2 := createTapIoctl(tap, uid); err2 != nil {
...
func createTapIoctl(tap string, uid uint64) error {
	...
	if err := unix.IoctlIfreq(fd, unix.TUNSETIFF, ifr); err != nil {
		return fmt.Errorf("TUNSETIFF %#q: %w", tap, err)
	}
	if err := unix.IoctlSetInt(fd, unix.TUNSETOWNER, int(uid)); err != nil {
		return fmt.Errorf("TUNSETOWNER %#q: %w", tap, err)
	}
	// Without this the device would disappear again when fd is closed.
	if err := unix.IoctlSetInt(fd, unix.TUNSETPERSIST, 1); err != nil {
		return fmt.Errorf("TUNSETPERSIST %#q: %w", tap, err)
	}
	return nil
}
...
// it, is writable by anyone but root. This helper runs as root, so a writable path
...
func verifySelf() error {
	...
	for path := exe; path != "/"; path = filepath.Dir(path) {
