...
// passed on the command line so that the sudoers file can pin it; the helper never
...
// ifNameRegex matches the interface names the kernel and a command line both
// accept. A leading dash is excluded because `ip` would parse it as an option,
// and "." and ".." are excluded because they are the /sys/class/net entries of
// the directory itself and its parent.
var ifNameRegex = regexp.MustCompile(`^[0-9a-zA-Z_.][0-9a-zA-Z_.-]{0,14}$`)
...
func validIfName(name string) bool {
	return name != "." && name != ".." && ifNameRegex.MatchString(name)
}
...
// terminated together with lima-privileged-net instead of leaking.
func (c *netConfig) startDnsmasq(ctx context.Context) (*exec.Cmd, error) {
	...
	// Without this, a crash of lima-privileged-net leaves a root dnsmasq behind.
...
func tapUp(ctx context.Context, tap, bridge string) error {
	...
	} else if err := createTap(ctx, tap, uid); err != nil {
		return err
	}
...
// nextIP returns the address right after ip, which validate() has already
// checked to be an IPv4 address.
func nextIP(ip net.IP) net.IP {
	addr, _ := netip.AddrFromSlice(ip.To4())
	return net.IP(addr.Next().AsSlice())
}
